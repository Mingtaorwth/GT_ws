...
def interpolate_transformations(transformation_matrices, new_time_stamps):
    ...
    # 插值位置数据
    pos_interpolator = interpolate.interp1d(timestamps, positions, axis=0, kind='linear', fill_value="extrapolate")
    interpolated_positions = pos_interpolator(new_time_stamps)

    # 插值四元数数据
    quat_interpolator = interpolate.interp1d(timestamps, quaternions, axis=0, kind='linear', fill_value="extrapolate")
    interpolated_quaternions = quat_interpolator(new_time_stamps)
    ...
    for t, pos, quat in zip(new_time_stamps, interpolated_positions, interpolated_quaternions):
        ...
        T = np.hstack((R_matrix, np.array([[pos[0]], [pos[1]], [pos[2]]])))  # 4x3
        T = np.vstack((T, [0, 0, 0, 1]))  # 形成 4x4 矩阵
...
def save_to_kitti_format(transformation_matrices, output_filename):
    with open(output_filename, 'w') as file:
        for _, T in transformation_matrices:
            ...
            # 将旋转矩阵和位移向量按一行输出
            row = R_matrix.flatten().tolist() + translation.tolist()  # 把旋转矩阵平铺成一行，并加上平移向量

            # 写入一行数据，确保每个数值之间有一个空格
...
if __name__ == "__main__":
    ...
    inter_num = 1183
    # 文件路径
    filename = "/home/mingtao/Gt_ws/gt_pose.txt"
    ...
    save_to_kitti_format(interpolated_transformations, "kitti_form_pose.txt")
